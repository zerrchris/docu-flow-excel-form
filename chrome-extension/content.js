...
// Access global snipSession from persistent-state.js

// Mass capture mode variables
let isMassCaptureMode = false;
let massCapturePanel = null;
let massCaptureStartRow = 0;
let massCaptureCount = 0;
...
async function addRowToSheet() {
  ...
  // Clear all input fields and textareas for next entry
      ...
      if (typeof window.snipSession !== 'undefined' && window.snipSession.active && typeof cleanupSnipSessionPreserveData === 'function') {
...
function hasUnsavedData() {
  ...
  // Check if there's meaningful data in textareas (excluding auto-generated values)
  ...
  for (const textarea of textareas) {
    ...
    if (value) {
      // Skip if it's just a document filename or auto-generated content
      const isDocumentName = textarea.getAttribute('data-column') === 'Document File Name' || 
                             value.match(/\.(jpg|jpeg|png|pdf|gif|bmp|tiff|webp)$/i);
      const isAutoGenerated = value.startsWith('Captured at ') || 
                              value.match(/^\d{4}-\d{2}-\d{2}/); // Date patterns
      
      if (!isDocumentName && !isAutoGenerated) {
        hasUserData = true;
        break;
      }
    }
...
function updateSnipCounter() {
  ...
  const totalSnips = (snipMode === 'navigate' || snipMode === 'scroll') ? 
    window.snipSession.captures.length : capturedSnips.length;
...
async function finishSnipping() {
  ...
  const snipsToProcess = (snipMode === 'navigate' || snipMode === 'scroll') ? 
    window.snipSession.captures : capturedSnips;
...
function showSnipPreview() {
  ...
  else if (typeof window.snipSession !== 'undefined' && window.snipSession.captures && window.snipSession.captures.length > 0) {
    const last = window.snipSession.captures[window.snipSession.captures.length - 1];
...
// Start mass capture mode
function startMassCaptureMode() {
  if (isMassCaptureMode || !activeRunsheet) return;
  
  console.log('🔧 RunsheetPro Extension: Starting mass capture mode');
  isMassCaptureMode = true;
...
function createMassCapturePanel() {
  if (massCapturePanel) return;
  
  massCapturePanel = document.createElement('div');
  ...
  // Add hover effects
  const nextBtn = massCapturePanel.querySelector('#mass-capture-next');
  const doneBtn = massCapturePanel.querySelector('#mass-capture-done');
  
  nextBtn.addEventListener('mouseenter', () => {
    nextBtn.style.background = 'rgba(255, 255, 255, 0.3) !important';
  });
  nextBtn.addEventListener('mouseleave', () => {
    nextBtn.style.background = 'rgba(255, 255, 255, 0.2) !important';
  });
  
  doneBtn.addEventListener('mouseenter', () => {
    doneBtn.style.background = 'rgba(220, 38, 38, 1) !important';
  });
  doneBtn.addEventListener('mouseleave', () => {
    doneBtn.style.background = 'rgba(220, 38, 38, 0.8) !important';
  });
...
function updateMassCapturePanel() {
  if (!massCapturePanel) return;
  
  const rowSpan = massCapturePanel.querySelector('#mass-capture-row');
  const countSpan = massCapturePanel.querySelector('#mass-capture-count');
...
async function endMassCaptureMode() {
  ...
  isMassCaptureMode = false;
  
  // Clean up any active snip mode
  cleanupSnipMode();
  
  // Remove the floating panel
  if (massCapturePanel) {
    massCapturePanel.remove();
    massCapturePanel = null;
  }
...
async function finishSnipSession() {
  ...
  if (window.snipSession.captures.length === 0) {
...
async function resetSnipSession() {
  ...
  if (runsheetFrame) {
    ...
    setupFrameEventListeners();
    // Ensure UI reflects latest captured snip
    setTimeout(() => {
